 ...
 * 红包准备页面, 设置发送红包的信息
 */
public class LuckyMoneyPrepareUIV7_0_9_1560 extends ActivityLifecycleHook {
    ...
    private void initView(Activity activity) {
        ...
        if (fromAppExt) {
            ...
            View parent = ViewUtils.findViewByIdName(activity, "d_q");
            // 总金额 d__,
            EditText totalEdit = (EditText) ViewUtils.findViewByIdName(parent, "d8f");
            totalEdit.setText(total);

            parent = ViewUtils.findViewByIdName(activity, "d_s");
            // 红包个数 d_b, id:d7z
            EditText numberEdit = (EditText) ViewUtils.findViewByIdName(parent, "d8f");
            numberEdit.setText(num);

            // 恭喜发财，大吉大利,  id:dak
            EditText descEdit = (EditText) ViewUtils.findViewByIdName(activity, "db1");
            if (!TextUtils.isEmpty(desc)) {
                descEdit.setText(desc);
            }

            // 塞钱进红包  id:d_m
            View confirmButton = ViewUtils.findViewByIdName(activity, "da3");
